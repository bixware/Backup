...
@Component({
  selector: 'app-addcalloutdetails',
  templateUrl: './addcalloutdetails.component.html',
  styleUrls: ['./addcalloutdetails.component.scss']
})
export class AddcalloutdetailsComponent implements OnDestroy, OnInit {
  apiURL: any;
  ...
  ngOnInit(): void {
    updatePageTitle('Add Callout Details');
    //hideSearchText();
    this.rowSelection = '';
...
$("#psearch").prop("disabled", false);
  ...
  const vehicleId = new Array();
    ...
    this.apiFlag = true;
    this.http.post<any>(this.apiURL + '/api/addvehiclecallout', this.updateVehicle)
      .subscribe(data => {
  ...
  patchCallout(): void{
    const htmlstring=" <div class='alert alert-danger' role='alert'> "+ 'Dummy Text' +'</div>' 
