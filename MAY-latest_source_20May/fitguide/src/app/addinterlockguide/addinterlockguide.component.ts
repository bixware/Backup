...
@Component({
  selector: 'app-addinterlockguide',
  templateUrl: './addinterlockguide.component.html',
  styleUrls: ['./addinterlockguide.component.scss']
})
export class AddinterlockguideComponent implements OnInit,OnDestroy {
  ...
  ngOnInit(): void {
    updatePageTitle('Add IGN Interlock');
    //hideSearchText();
  ...
  updateVehicleDetails(): void {
    ...
    if (this.form.value.headers.length === 0) {
      ...
      if (this.form.value.headers[0].functionName != '') {
        arrayString = '[';
    ...
    this.updateVehicle.vehicleIGNPartDetails = arrayString.replace('null', '');
    console.log(this.updateVehicle);
    this.http.post<any>(this.apiURL + '/api/updatevehicledetailsinterlockpart', this.updateVehicle)
      .subscribe(data => {
  ...
  close1(): void {
    this.apiFlag = false;
    this.router.navigate(['/home/igninterlockguide']);
  }
