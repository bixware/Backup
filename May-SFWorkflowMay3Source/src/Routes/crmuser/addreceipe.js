...
const AddReceipe = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    form_data.append("receipeName", formData.receipeName);
    form_data.append("FGCode", formData.fGCode);
    ...
    try {
      const response = await apiFormDataPost("user/sfcreaterc", form_data);
      if (response) {
        setSuccess(true);
        setLoading(false);
      } else {
  ...
  return (
    <RctCollapsibleCard>
      ...
      <SweetAlert
        ...
        onConfirm={() => onConfirm()}
      >
        Receipe Creation added Successfully!
      </SweetAlert>
    </RctCollapsibleCard>
  );
};
