    ...
    public function AmexData(Request $request) {
        ...
        try {
            for ($i = $this->repository->startFrom; $i <= $arrayCount; $i++) {
                ...
                $deposit_dt = strtotime($deposit_dt);
                $deposit_dt = date('Y-m-d', $deposit_dt);

                $storedata = ExcelUploadGeneralService::getReteckCodeUsingTIDForAMEX($mid, Carbon::parse(trim(str_replace("'", '', $deposit_dt))));
                ...
                if ($deposit_dt != '' && $cr_dt != '') {
                    if (MFLInwardCardMISAmexPos::updateOrInsert($attributes, $data)) {
