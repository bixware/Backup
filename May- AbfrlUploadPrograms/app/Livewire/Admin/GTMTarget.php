...
use App\Models\M_GTMTarget;
use App\Models\mGTMTarget;
...
class GTMTarget extends AdminComponent
{
    ...
    {
        return DB::withOrderBySelect('UploadProgram_PROC_SELECT_ADMIN_GTM_TARGET :procType,:user, :role,:sapcode,:storename,:startDate,:endDate', [
            "procType" => 'ADMIN',
            "user" => Auth::id(),
            "role" => 1,
    ...
    public function render()
    {
        ...
        return view('livewire.admin.g-t-m-target', [
    ...
    {
        $this->validate();
        $this->fileupload->storeAs('GTM Target', $this->fileupload->getClientOriginalName());
        $getOriginalFilename = $this->fileupload->getClientOriginalName();
        $targetPath = storage_path('app/GTM Target/' . $getOriginalFilename);
                ...
                for ($i = 2; $i <= $arrayCount; $i++) {
                    ...
                    if ($SAPCode != '' && $storeName != '' && $date != '' && $day != '' && $WKNO != '' && $OPNSVTarget != '' && $walkins != '' && $bills != '' && $mensVol != '' && $womensVol != '' && $totalVol != '' && $GSV != '' && $MRP != '' && $userId != '') {
                        ...
                        DB::transaction(function () use ($data, $attributes) {
                            mGTMTarget::updateOrInsert($attributes, $data);
                            DB::commit();
                        });
                    }
                }
